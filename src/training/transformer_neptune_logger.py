# detailed metrics Neptune logger for transformer models 
...
class TransformerNeptuneLogger:
    """Neptune logger for transformer models with detailed tracking - matching LSTM structure."""
    
    def __init__(self, experiment_tracker, model, train_data, val_data, metadata_train, metadata_val):
        """Initialize the transformer Neptune logger."""
        self.tracker = experiment_tracker
        self.model = model
        self.train_data = train_data  # (X_train, masks_train, Y_train)
        self.val_data = val_data  # (X_val, masks_val, Y_val)
        ...
        print(f"TransformerNeptuneLogger init:")
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        train_acc = logs.get('train_acc', 0)
        ...
        if any(epoch == e[1] for e in self.top_epochs):
            ...
            torch.save(self.model.state_dict(), model_filename)  # save the model weights
    ...
    def _compute_detailed_accuracies(self):
        ...
        X_train, mask_train, Y_train = self.train_data
        X_val, mask_val, Y_val = self.val_data
        ...
        for category in categories:
            ...
            if is_df_metadata and category_col in self.metadata_train.columns:
                    ...
                    try:
                        ...
                        if len(train_indices) > 0 and len(val_indices) > 0:
                            ...
                            with torch.no_grad():
                                ...
                                mask_train_subset = mask_train[train_indices]
                                Y_train_subset = Y_train[train_indices]
                                
                                train_outputs = self.model(X_train_subset, mask_train_subset)
                                if train_outputs.dim() > 1 and train_outputs.size(1) == 1:
                                ...
                                mask_val_subset = mask_val[val_indices]
                                Y_val_subset = Y_val[val_indices]
                                
                                val_outputs = self.model(X_val_subset, mask_val_subset)
                                if val_outputs.dim() > 1 and val_outputs.size(1) == 1:
                                    val_outputs = val_outputs.squeeze(1)
                                val_preds = (torch.sigmoid(val_outputs) > 0.5).float()
                                val_acc = (val_preds == Y_val_subset).float().mean().item()
                ...
                with torch.no_grad():
                    train_outputs = self.model(X_train, mask_train)
                    if train_outputs.dim() > 1 and train_outputs.size(1) == 1:
                        train_outputs = train_outputs.squeeze(1)
                    train_preds = (torch.sigmoid(train_outputs) > 0.5).float()
                    train_acc = (train_preds == Y_train).float().mean().item()
                    
                    val_outputs = self.model(X_val, mask_val)
                    if val_outputs.dim() > 1 and val_outputs.size(1) == 1:
                        val_outputs = val_outputs.squeeze(1)
                    val_preds = (torch.sigmoid(val_outputs) > 0.5).float()
